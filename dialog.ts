namespace story {
    ...
    class ConversationState {
        ...
        showMenu(choices: string[]) {
            ...
            const menuHeight = Math.max(arrowIcon.height, arrowText.height) + 4;
            ...
            arrowIcon.top = screen.height - menuHeight + ((menuHeight - arrowIcon.height) >> 1);
            arrowText.top = screen.height - menuHeight + ((menuHeight - arrowText.height) >> 1);
            buttonIcon.top = screen.height - menuHeight + ((menuHeight - buttonIcon.height) >> 1);
            buttonText.top = screen.height - menuHeight + ((menuHeight - buttonText.height) >> 1);

            const backdrop = new story.RectangleSprite(story.TEXT_Z);
            backdrop.setDimensions(Math.idiv(screen.width - 4, UI_SCALE), Math.idiv(menuHeight, UI_SCALE));
            backdrop.top = screen.height - menuHeight;
    ...
    export function printCharacterText(text: string, label?: string) {
        ...
        const task = printDialog(text, x, y, height, width);

        if (label) {
            const padding = 1;
            const labelText = new story.TextSprite(story.TEXT_Z + 2);
            labelText.setText(label);
            labelText.setColor(15);
